...
class library{
    ...
    public $nameColHot_productCat = 'productcat_hot';
    public $nameColNew_productCat = 'productcat_new';
...
//            $results = $stmt->get_result();
	...
	/*--------------- Danh mục đệ quy ---------------*/
		...
		function getTypePage_byUrl($urlFriendly,$lang){
			...
			/*--- Kiểm tra Url có thuộc danh sách bài học không? --*/
			$type = 0;
		    $sql = "SELECT * from color where friendly_url =  '".$urlFriendly."'";
		    $result = mysqli_query($conn_vn,$sql);
		    $total_record = mysqli_num_rows($result);
		    $type = $type + $total_record;		   
		    if ($type == 1) { 
