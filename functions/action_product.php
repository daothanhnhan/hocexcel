...
public function getColorDetail_byUrl($url,$lang){
    global $conn_vn;
    $sql = "SELECT * from color where friendly_url = '".$url."'";    
    $result = mysqli_query($conn_vn,$sql);    
    if (mysqli_num_rows($result) > 0) {
        $row = mysqli_fetch_assoc($result);
        return $row;
...
/*---------- Liet ke tat ca bài học  theo id ------------*/
public function getListColor_byOrderASC($idProduct){
    global $conn_vn;

    $rows = array();
    $sql = "SELECT * FROM color where product_id = $idProduct  order by color_id ASC";  
    // echo $sql;
...
/*------ Khóa học nổi bật -----*/
public function getListProductCatHot_hasLimit($limit){
    global $conn_vn;
    $table = $this->nameTable_productCat;
    $where = "where $this->nameColHot_productCat != 0";
    $limit = "limit ".$limit;
    $sql = "SELECT * from $table $where $limit";    
    // echo $sql;
    $result = mysqli_query($conn_vn,$sql);
...
/*---------- Liệt kê một số sản phẩm show ở trang chủ - Có giới hạn ------------*/
public function getListProductHome_HasLimit($limit){
    global $conn_vn;
    $rows = array();
    $sql = "SELECT * FROM product where state = '1'  order by product_id LIMIT $limit";  
...
public function getTeacher ($id) {
    global $conn_vn;
    $sql = "SELECT * FROM teacher WHERE id = $id";//echo $sql;
    $result = mysqli_query($conn_vn, $sql);
    $num = mysqli_num_rows($result);
    if ($num > 0) {
        $row = mysqli_fetch_assoc($result);
