...
class action_cart extends library{
  ...
  /*-------------------------------------------------*/
  public function addCart($total_price, $session_id, $name_buyer, $email_buyer, $phone_buyer, $address_buyer, $note_buyer, $code_active){

    global $conn_vn;
    $date_cart = $this->sw_get_current_weekday();
    $sql = "INSERT INTO cart (session_id, total_price, date_cart, name_buyer, mail_buyer, phone_buyer, address_buyer, note_buyer, code_active) values ('".$session_id."',"."'".$total_price."',"."'".$date_cart."',"."'".$name_buyer."',"."'".$email_buyer."',"."'".$phone_buyer."',"."'".$address_buyer."',"."'".$note_buyer."', "."'".$code_active."')";
    $result = mysqli_query($conn_vn,$sql);  
  ...
  /*-------------------------------------------------*/  
  public function payment1 ($code_active) {
      if (!isset($_SESSION['shopping_cart']) || $_SESSION['shopping_cart'] == '') {
          echo json_encode(array('error'=>'Giỏ hàng trống'));
      ...
      $session_id = session_id();    
      $id_cart = $this->addCart(0, $session_id, $name_buyer, $email_buyer, $phone_buyer, $address_buyer, $note_buyer, $code_active);
      foreach ($rows as $row) {
          $this->addItemCart($row->id_product, $row->count, $row->price, $session_id, $row->color, $row->size, $row->id_color, $row->id_size, $id_cart);
          $totalPrice += $row->total;              
      }
      if($this->updateCartTable($totalPrice,$id_cart)){
          $this->emptyCart();
          session_regenerate_id();
          echo json_encode(array("success"=>"Gửi đơn hàng thành công. Mời bạn dùng mã đã được nhận để kích hoạt khóa học", "totalCart"=>$this->countCart(), "url"=>"/kich-hoat"));
